...
window.addEventListener('scroll', () => {
    ...
    const scrollThreshold = maxScroll * 0.18; // Define the scroll threshold (e.g., 90% of the page height)
...
window.addEventListener('resize', () => {
    const patternLeftPercentage = 85; // Left percentage of the background pattern
    const patternTopPercentage = 186; // Top percentage of the background pattern

    const imageWidth = 150; // Width of the zooming image
    const imageHeight = 125; // Height of the zooming image
    ...
    const zoomingImage = document.getElementById('zooming-image');
    zoomingImage.style.top = `${topPosition}px`;
    zoomingImage.style.left = `${leftPosition}px`;
    zoomingImage.style.transform = `translate(-50%, -50%)`; // Center the image
});
